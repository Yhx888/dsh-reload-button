...
/**
 * 调度后端重启。systemctl 是独立 D-Bus 客户端：旧 unit 被 SIGTERM 前
 * 请求已入队 systemd，因此本进程死亡不影响重启继续（延迟自杀）。
 */
function scheduleRestart(ctx) {
  const child = spawn('systemctl', ['restart', 'dsh-web.service'], {
    detached: true,
    stdio: 'ignore',
  })
  child.on('error', (error) => {
    const log = (ctx.logger || console)
    log.warn(`[dsh-reload-button] systemctl restart 启动失败：${error.message}`)
  })
  child.unref()
}
